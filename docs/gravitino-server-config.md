...
| `gravitino.entityChangeLog.pollLagSecs`            | The lag in seconds applied to the entity change log poller. Only change rows older than this (by the database clock) are consumed, so all transactions that started before a consumed row have committed. This closes the auto-increment id commit-ordering gap where a lower id may commit after a higher id. Should exceed the longest write transaction. Set to `0` to disable. Must be non-negative. | `1`                               | No                                              | 1.4.0            |
