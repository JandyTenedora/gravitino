...
| `gravitino.entityChangeLog.pollLagSecs`            | The lag in seconds applied to the entity change log poller. Only change rows whose `created_at` is at least this many seconds in the past (by the database clock) are consumed. Auto-increment ids are assigned at INSERT but become visible at COMMIT, so a lower id may commit after a higher id and be skipped by an id-only cursor; waiting until a row is this old gives the inserting transaction of any earlier (smaller-id) row time to commit before the cursor advances past it, reducing missed invalidations. Should exceed the longest write transaction. Set to `0` to disable. Must be non-negative. | `1`                               | No                                              | 1.4.0            |
